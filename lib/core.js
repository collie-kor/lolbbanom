...
규칙:
- 위 4가지 중 실제로 사진에서 관찰되는 것만 후보(candidates)로 고르세요. 없으면 빈 배열.
- 최대 4개, 최소 0개.
...
- 위 4가지에 해당하지 않지만 사진에서 관찰되는 다른 접근성 장애 요소가 있으면 extra 배열에 넣으세요. label(15자 이내 짧은 제목)과 reason(관찰 문장). 최대 2개, 확실하지 않으면 넣지 마세요. 여기서도 판정하지 말고 관찰만 하세요.

아래 JSON 형식으로만 답하세요:
{"sceneRecognized": true, "candidates": [{"id": "curb", "reason": "출입구 앞에 낮은 단차가 보입니다"}], "extra": [{"label": "통로 위 자전거", "reason": "통로 옆에 자전거가 세워져 있어 폭이 좁아 보입니다"}]}`;
  ...
  try {
    ...
    const extra = Array.isArray(parsed.extra)
      ? parsed.extra
          .filter((e) => e && e.label)
          .slice(0, 2)
          .map((e) => ({ label: String(e.label).slice(0, 40), reason: String(e.reason || "").slice(0, 160) }))
      : [];

    return {
      status: 200,
      body: { ok: true, sceneRecognized: parsed.sceneRecognized !== false, candidates, extra },
    };
  } catch (err) {
...
// ── 작성: 확인된 정보 → 요청문 초안 ────────────────────────
export async function compose({ receiverId, obstacleId, obstacleLabel, obstacleReason, userIds, improvement, location }) {
  const receiver = rules.receivers[receiverId];
  const obstacle = obstacleId ? rules.obstacles[obstacleId] : null;
  // 규칙표 장애물이면 그 라벨을, 커스텀(직접입력·AI 자유발견) 항목이면 전달된 라벨을 사용
  const obLabel = obstacle ? obstacle.label : (obstacleLabel ? String(obstacleLabel).slice(0, 80) : "");
  const obReason = obstacle ? "" : String(obstacleReason || "").slice(0, 200);
  if (!receiver || !obLabel || !improvement) {
...
function buildFallbackLetter({ receiver, obLabel, userLabels, phrases, improvement, loc }) {
  const locLine =
  ...
  return (
    ...
    locLine +
    `해당 공간의 출입 동선에서 ${obLabel}이(가) 관찰되었습니다.${impact}\n\n` +

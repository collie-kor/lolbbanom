...
# .env 에 GEMINI_API_KEY 를 채웁니다 (아래 참고). 없어도 폴백으로 동작.
npm start
```
...
2. Settings > Environment Variables 에 `GEMINI_API_KEY` 등록 후 **재배포**
3. 배포판에서 사진 → 위치 입력 → 요청문까지 **한 번 끝까지** 돌려본다
4. 제출 최소 2시간 전 배포하고 실기기에서 확인. Serverless 타임아웃(기본 10초) 대응으로 Gemini 호출은 8초 자체 중단 처리됨.
